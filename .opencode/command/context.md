...
**utils/house-settings.ts** — getters cached com fallback aos defaults de `src/utils/settings.ts`: `getHouseRewardPricingSettings`, `getHouseQuickMessageSettings`, `getHouseTaskSlaSettings` (`defaultDueDays`/`dueSoonHours`), `getHouseExtensionRulesSettings` (`dayOptions`), `getHouseNotificationRetentionSettings` (`readRetentionDays`), e `getHouseSettingsValue(houseId, key)` interno. Lidos por `/dashboard/admin/settings` e repassados às páginas (`/tasks` → `TasksAdmin`/`TasksDependent`; call sites DEPENDENT da mensagem rápida).
