...
**utils/task-decay.ts** — `getTaskCurrentPoints(basePoints, createdAt, dueDate, settings, now = new Date())` → valor corrente de `tasks.points` sob o decaimento da casa (perde `pointsPerPeriod` a cada `periodHours` completas desde a criação, janela **capada no `due_date`** — depois do prazo a perda não cresce —, **piso em 0**). Isomórfico (sem deps de servidor) — usado nas actions (`approveTask`/`adminCompleteTask` creditam o valor corrente; `markTaskNotDelivered` debita; `resolveTaskExtension`/`updateTask` restauram o mesmo) e nos cards ADMIN/dependente (pill mostra valor corrente com a base riscada quando reduzido; aprovadas mostram a base, histórico). Ver topo do `PROJECT_STATUS.md`.
