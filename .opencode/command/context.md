...
- **Migrações SQL não versionadas:** pasta `supabase/` não existe (`supabase/*.sql` gitignored) — mudanças de schema são aplicadas manualmente no dashboard Supabase. **Todos os scripts já documentados foram aplicados** (imagens, `reward_suggestions`, `extension_*`, enum `task_status` com `NOT_DELIVERED`, tabela `notifications` + policy de SELECT + publication `supabase_realtime`). **Exceções a aplicar manualmente:** a coluna `rewards.active` (desativação de recompensa) ainda **não** foi aplicada; as colunas `notifications.image_url`/`message_id` (mensagem rápida) só devem ser aplicadas **ao ativar a feature** (ver seção "Mensagem rápida" do `PROJECT_STATUS.md`).
...
**rewards** — `id`, `house_id`, `active` (bool, default true; `false` = desativada — indisponível, nunca excluída; guard em `requestRedemption`; **coluna pendente de aplicação no Supabase**), `title`, `description`, `points_cost`, `emoji`, `image_url`, `created_by`, `created_at`, `updated_at`.
...
- **Mensagem rápida (DEPENDENT → ADMINs):** `sendQuickMessage(text, imageUrl?)` — só DEPENDENT; texto ≤50 caracteres opcional (exige texto OU imagem) e imagem = URL pública do bucket (`messages/`); **capacidade**: envia apenas se não tiver >2 mensagens próprias acumuladas (lidas ou não; conta `message_id` distintos, não cópias); insere 1 cópia por ADMIN da casa com o mesmo `message_id`. Ao abrir uma `QUICK_MESSAGE` no sino o visualizador marca como lida (sem link); **retenção** "2 lidas → apaga a mais antiga" em `cleanupQuickMessages` (disparado em `markNotificationRead`/`markAllNotificationsRead`; apaga todas as cópias + imagem no storage). Composer no sino só do DEPENDENT (`quick-message-composer.tsx`: galeria + câmera ao vivo `getUserMedia` com fallback). Constantes em `src/utils/quick-message.ts` (`QUICK_MESSAGE_MAX_CHARS=50`, `MAX_IMAGE_MB=5`, `CAPACITY=2`).
...
**utils/notifications.ts** — `READ_RETENTION_DAYS` (5), `notifyUser(admin, {...})`, `notifyHouse(admin, { side: 'ADMINS'|'DEPENDENTS', excludeUserId, ... })` (ambas com passthrough opcional de `imageUrl`/`messageId`), `cleanupReadNotifications(admin, recipientId)`, `getMyNotifications(userId)` (limpeza lazy + lista), `cleanupQuickMessages(admin, houseId, actorId)` (retenção "2 lidas → apaga a mais antiga" das QUICK_MESSAGE + imagem do storage). Best-effort (service-role).
