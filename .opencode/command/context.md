...
- **Migrações SQL não versionadas:** pasta `supabase/` não existe (`supabase/*.sql` gitignored) — mudanças de schema são aplicadas manualmente no dashboard Supabase. **Todos os scripts já documentados foram aplicados** (imagens, `reward_suggestions`, `extension_*`, enum `task_status` com `NOT_DELIVERED`, tabela `notifications` + policy de SELECT + publication `supabase_realtime`). **Exceção única:** a coluna `rewards.active` (desativação de recompensa) ainda **não** foi aplicada.
