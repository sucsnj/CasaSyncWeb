...
### Pontos de atenção
- **SQL aplicado pelo usuário no Supabase** (as tabelas, a RLS e a publication); faltava apenas a **policy de SELECT** `comunicados_select_members` (não veio no primeiro run — ver bloco de garantia na subseção abaixo).
...
### Ajuste — entrega instantânea + hydration do overlay (concluído)
- **Instante, não: era a policy de SELECT que faltava no banco.** Com o SQL aplicado, o balão ainda só aparecia em refresh/navegação. Diagnóstico (Fase 1): `pg_policies` não tinha `comunicados_select_members` — o Realtime aplica RLS pelo papel de quem assina e `SUBSCRIBED` chega mas o evento é descartado em silêncio (causa nº 1 da doc do Supabase); o render do servidor via service role ignora RLS, por isso refresh/navegação mostravam o aviso. Fix: rodar o **bloco de garantia idempotente** (`do $$` com publication membership + `create policy` — docs da seção adjacente), depois validar ao vivo o log `[Realtime] Conectado: comunicados (house_id=eq.…)`. `replica identity 'd'` (default) foi confirmado e **não** bloqueia entrega (só afeta `payload.old`).
- **Bug de hydration corrigido (`comunicado-overlay.tsx`):** o guard `if (!current || typeof document === 'undefined') return null` (branch `typeof window` — causa listada na msg do React) renderizava `null` no SSR e o portal no cliente → árvores divergiam sempre que a fila tinha item (quebrou `/tasks` do dependente ao testar). Troca pela leitura `isMounted = useSyncExternalStore(emptySubscribe, () => true, () => false)` — padrão idêntico ao `FormattedDateTime`: servidor e hidratação veem `false`, só depois o portal monta. Sem efeito (o linter `react-hooks/set-state-in-effect` rejeita) e sem impacto na fila/Realtime.
- **Verificação:** `npm run lint` ✓ (só warnings `no-img-element`/`House` esperados) · `npm run typecheck` ✓ · `npm run build` ✓ (13 rotas, `ƒ Proxy` ativo).
- **Pendente de confirmação (banco):** rodar o bloco de garantia e o teste ao vivo. Se a entrega ao vivo voltar, **não haverá Fase 2** (sinal via `notifications` fica descartado).

### Fase 2 — entrega ao vivo garantida por notificação (concluída)
- **Fase 1 não resolveu:** mesmo com a policy `comunicados_select_members` e a publication aplicadas no banco, o Realtime direto de `comunicados` **continuou não entregando** ao navegador do dependente (frustração registrada: "ao vivo ainda não funciona"). Em vez de insistir num canal cujo comportamento no Supabase (WAL + RLS + ingest) segue opaco, a entrega da 1ª exibição passou a usar o **canal já comprovado neste app**: `notifications`.
- **`setComunicadoPublished` publicou → notifica os DEPENDENTEs:** após o `UPDATE` com `published = true`, `notifyHouse` (best-effort, service-role) insere uma notificação `COMUNICADO_PUBLISHED` por dependente da casa (título "Novo comunicado da casa", body = título do aviso, `excludeUserId` = o ADMIN que agiu). Ganha sino + toast (`RealtimeToastListener`) + **Web Push** automaticamente.
- **Overlay escuta o sinal:** `ComunicadoOverlay` ganhou a prop **`userId`** e uma 2ª subscription (`notifications`, filter `recipient_id=eq.<userId>`, `event: INSERT`); ao receber `COMUNICADO_PUBLISHED` reexecuta `getDueComunicados()` — a fila fecha o balão na hora, sem depender do Realtime de `comunicados`. A subscription direta de `comunicados` foi **mantida** (cobre edição/despublicação/exclusão quando o canal entrega; o servidor segue a autoridade do que é "devido").
- **Novo tipo de notificação:** `COMUNICADO_PUBLISHED` adicionado à união em `src/types/notifications.ts`, ao `TYPE_STYLE` do toast (info) e ao `TYPE_META` do sino (`Megaphone` violeta — mesmo tom do overlay). Sem mudança de schema (a coluna `notifications.type` não tem CHECK).
- **Montagens atualizadas:** `userId` repassado nos 4 render points acima (layout dependente e branches de `/tasks`, `/rewards`, `/achievements`).
- **Verificação:** `npm run lint` ✓ (só warnings esperados) · `npm run typecheck` ✓ · `npm run build` ✓ (13 rotas, `ƒ Proxy` ativo).
- **Pendências:** rodar `npm run dev` com o `.next` limpo (build já regenerado — o erro de hydration no `DashboardNav` era build misto: servidor renderizando HTML ao lado antigo com bundle novo) e testar ao vivo: publicar no ADMIN e ver o balão + toast/sino no dependente, conferindo no console os logs `[Realtime] Conectado: notifications (recipient_id=eq.…)` e `[notifyHouse] Inserted successfully`. Produção exige deploy.
