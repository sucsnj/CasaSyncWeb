...
- **Notificações:** sino no cabeçalho azul para "o outro lado" da ação (dependente ↔ ADMINs), marcar como lida/todas, apagar uma/todas; lidas são apagadas após 5 dias. **Mensagem rápida** do DEPENDENT para os tutores (texto ≤50 caracteres + até 1 imagem da galeria ou câmera ao vivo, retenção "2 lidas → apaga a mais antiga"). Ver ADR-0009.
...
- **Uploads:** bucket público `casasync-media` (avatars, casas, recompensas, tarefas, sugestões + `messages` da mensagem rápida).
